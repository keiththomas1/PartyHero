...
public class HomeScreenController : MonoBehaviour {
    ...
    public void EnterScreen()
    {
        ...
        tapLabel = GameObject.Instantiate(Resources.Load("Prefabs/SocialMedia/Home/TapLabel") as GameObject);

        currentAnnoyTimer = 0.05f;
        clickMultiplier = serializer.IsDoubleClickEnabled() ? 2 : 1;
    }

    private void CreateAnnoyanceMeter()
    {
        annoyMeter = GameObject.Instantiate(Resources.Load("Prefabs/SocialMedia/Home/AnnoyanceMeter") as GameObject);
        var annoyMeterPosition = annoyMeter.transform.position;
        annoyMeterPosition.x = 0.0f;
        annoyMeterPosition.y = -0.5f;
    ...
    {
        post = GameObject.Instantiate(Resources.Load("Prefabs/SocialMedia/Home/DGHomePage") as GameObject);
        var postPosition= transform.parent.position;
        postPosition.y += 1.3f;
        ...
        if (likesText)
        {
            likesText.GetComponent<TextMesh>().text = GetRandomLikes();
        }
    ...
    private void CheckClickOutcome()
    {
        if (currentClickCount >= 1.0f)
        {
            ...
            var newFollowerObject = GameObject.Instantiate(Resources.Load("Prefabs/SocialMedia/Home/HomeFollowerText") as GameObject);
            var followerObjectPosition = newFollowerObject.transform.position;
            followerObjectPosition.x = 0.0f;
            followerObjectPosition.y = 2.5f;
            newFollowerObject.transform.position = followerObjectPosition;

            var newFollowerText = newFollowerObject.transform.Find("DGFollowerText").gameObject;
            newFollowerText.GetComponent<TextMesh>().text = "+" + newFollowers + " Follower";
    ...
    private void RemovePost(GameObject post)
    {
        ...
        post.GetComponent<RotateAway>().StartAnimation();
    }
